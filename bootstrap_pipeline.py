...
def start_pipeline_server(vpy: Path) -> int:
    ...
    proc = subprocess.Popen(
        [str(vpy), str(b.ROOT / "pipeline_server.py")],
    ...
    if b.SERVER_ERR.exists():
        tail = "\n".join(b.SERVER_ERR.read_text(encoding="utf-8", errors="replace").splitlines()[-100:])
    raise RuntimeError("Backend failed to start.\n" + tail)


b.start_server = start_pipeline_server
